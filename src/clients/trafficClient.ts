...
export class TrafficClient {
  private static vendor: string = Vendor.Traffic;
  private static baseUrl: string = BaseUrl.Traffic;
  private static products: Array<IProduct> = new Array<IProduct>();
  private static factory: TrafficScraper = new TrafficScraper();
  private static config: IConfig = config;

  public static async run(): Promise<void> {
    const shopifyApi = new ApiService(VendorApiUrl.Traffic);
