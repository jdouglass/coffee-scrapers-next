...
export class HouseOfFunkClient {
  private static vendor: string = Vendor.HouseOfFunk;
  private static baseUrl: string = BaseUrl.HouseOfFunk;
  private static products: Array<IProduct> = new Array<IProduct>();
  private static factory: HouseOfFunkScraper = new HouseOfFunkScraper();
  private static config: IConfig = config;

  public static async run(): Promise<void> {
    const shopifyApi = new ApiService(VendorApiUrl.HouseOfFunk);
