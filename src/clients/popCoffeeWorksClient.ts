...
export class PopCoffeeWorksClient {
  private static vendor: string = Vendor.PopCoffeeWorks;
  private static baseUrl: string = BaseUrl.PopCoffeeWorks;
  private static products: Array<IProduct> = new Array<IProduct>();
  private static factory: PopCoffeeWorksScraper = new PopCoffeeWorksScraper();
  private static config: IConfig = config;

  public static async run(): Promise<void> {
    const shopifyApi = new ApiService(VendorApiUrl.PopCoffeeWorks);
...
const main = async (): Promise<void> => {
  try {
    await PopCoffeeWorksClient.run();
  } catch (error) {
    console.error(error);
  }
};
